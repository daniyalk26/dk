...
BASE_URL = "http://18.119.104.127:8501"  # Update if needed
...
def display_grid(items, item_type="artist", columns_per_row=3):
    ...
    Force a uniform width=160 to keep images from scaling too large.
    """
    if not items:
        st.info(f"No {item_type} data found.")
        return

    # Break items into rows, each row up to columns_per_row items
...
def extract_spotify_data(sp):
    ...
    # Save locally & upload to raw S3 bucket (for Lambda processing)
...
def main():
    ...
    **Connect to see your**:
    ...
    # 1) Create Spotipy OAuth object (manual approach)
    ...
    # 2) Check if we have a "code" parameter in the URL
    query_params = st.query_params
    code_param = query_params.get("code", None)

    # 3) If no code yet, show link to Spotify login
    if code_param is None:
        ...
        # 4) Exchange code for a token (silently, no extra logs)
        ...
        # Optional: if your Lambda triggers automatically on S3 put
        # we wait a little to let Lambda do the transformation
        time.sleep(5)
        ...
        # 7) RAW & PROCESSED DATA SECTIONS AT BOTTOM
